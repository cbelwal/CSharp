...
namespace Dasmic.MLLib.Algorithms.DecisionTree
{
    public abstract class BuildBase : Common.MLCore.BuildBase
    {
        ...
        protected virtual DecisionTreeNode buildChildNodes(ConcurrentBag<int> trainingDataRowIndices,
                                                   double value,
                                                      ...
                                                      ConcurrentBag<int> trainingDataRowIndices)
        {
            ...
            ConcurrentBag<int> bagTrainingDataRowIndices = 
            ...
            int[] arrayTrainingDataRowIndices = bagTrainingDataRowIndices.ToArray<int>();
        ...
        /// <param name="trainingDataRowIndices"></param>
        /// <returns></returns>
        protected FilteredData convertRowIndicesToFilteredData(ConcurrentBag<int> trainingDataRowIndices)
        {
            double[][] filteredData = Get2DArray(_trainingData.Length, trainingDataRowIndices.Count);
            int idx = 0;
            foreach (int rowIdx in trainingDataRowIndices)
            //Parallel.ForEach(trainingDataRowIndices, new ParallelOptions { MaxDegreeOfParallelism = _maxParallelThreads }, rowIdx =>
            {
                for (int colIdx = 0; colIdx < _trainingData.Length; colIdx++)
                {
