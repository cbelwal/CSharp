...
namespace Dasmic.MLLib.Algorithms.InstanceBased
{
    public class ModelSelfOrganizingMap:ModelBase
    {
        ...
        /// <returns></returns>
        public 
          void RunModelForSingleData(double[] data, ref int X, 
                                                        ref int Y)
        {
            ...
            Parallel.For(0, data.Length,
                       ...
                       //for (int xdim = 0; xdim < data.Length; xdim++) //Parallelize this loop
            {
                double computedDistValue = double.MaxValue;
                for (int ydim = 0; ydim < data.Length; ydim++)
                {
                                                            ...
                                                            string [] names)
        {
            ...
            Parallel.For(0, data.Length,
                       new ParallelOptions { MaxDegreeOfParallelism = _maxParallelThreads }, xdim =>
            //for (int xdim = 0; xdim < SomMap.Length; xdim++) //Parallelize this loop
            {
                double computedDistValue = double.MaxValue;
                string tmpLine = "";
                for (int ydim = 0; ydim < SomMap[0].Length; ydim++)
