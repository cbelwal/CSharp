...
namespace Dasmic.MLLib.Algorithms.NeuralNetwork
{
    ...
    /// </summary>
    public class BuildGenericBackPropagationSGD : BuildBackPropagationBase
    {       
        IActivationFunction _activationFunctionOutputLayer;

        public BuildGenericBackPropagationSGD()
        {
                        ...
                        int indexTargetAttribute)
        {
            ...
            learningRate = _alpha;//Use constant rate - R also keeps learning rate same                
            //Start iterations
            for (int epoch = 0; epoch < _noOfEpoch; epoch++) //Do not parallelize
            {
                ...
                for (int row = 0; row < _noOfDataSamples; row++) //For each data sample
                {
                    ...
                    //Update weight for each row of training data using Stoicastic Gradient Descent
                                    ...
                                    int row)
        {
                    ...
                    idxLayer =>
            {
                ...
                int noOfUpstreamUnits = _model.GetNumberOfUpstreamUnits(idxLayer);
